...
from gomoku_rl.utils.reward_shaping import compute_shaped_reward
...
def make_transition(
    ...
    if use_shaping:
        shaped_reward = compute_shaped_reward(
            tensordict_t_minus_1, tensordict_t, gamma=0.99, scale=0.0001)
        reward += shaped_reward.unsqueeze(-1)
...
def fake_round(env: GomokuEnv,
          policy_black: _policy_t,
          policy_white: _policy_t,
          policy_white_1:_policy_t,
          policy_black_1:_policy_t,
          tensordict_t_minus_1: TensorDict,
          tensordict_t: TensorDict):
    """Executes two sequential steps in the Gomoku environment, applying black and white policies alternately.
    ...
    tensordict_t_plus_1 = env.step_and_maybe_reset(
        tensordict=tensordict_t)
    current_policy= random.choice([policy_white,policy_white_1])
    with set_interaction_type(type=InteractionType.RANDOM):
        tensordict_t_plus_1 = current_policy(tensordict_t_plus_1)
    ...
    current_policy= random.choice([policy_black,policy_black_1])
    with set_interaction_type(type=InteractionType.RANDOM):
        tensordict_t_plus_2 = current_policy(tensordict_t_plus_2)
...
class FakeCollector(Collector):
    def __init__(self, env: GomokuEnv, policy_black: _policy_t, policy_white: _policy_t, policy_white_1:_policy_t, policy_black_1:_policy_t, out_device=None, augment: bool = False):
        """Initializes a collector for versus play data in a Gomoku environment, facilitating the collection of game transitions where two players, each using a distinct policy, compete against each other.
        ...
        self._policy_black = policy_black
        self._policy_white = policy_white
        self._policy_white_1 = policy_white_1
        self._policy_black_1 = policy_black_1
        self._out_device = out_device or self._env.device
        self._augment = augment
        self._t_minus_1 = None
        self._t = None
    ...
    @torch.no_grad()
    def rollout(self, steps: int) -> dict:
        """Executes a rollout in the environment, collecting data for a specified number of steps, alternating between the black and white policies.
        ...
        for i in range(steps//2):
            (
                self._t_minus_1,
                self._t,
            ) = fake_round(self._env, self._policy_black, self._policy_white,self._policy_white_1, self._policy_black_1,self._t_minus_1, self._t)
