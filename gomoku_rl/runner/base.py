...
class Runner(abc.ABC):
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.policy_black_1 = get_policy(
            name=cfg.algo.name,
            cfg=cfg.algo,
            action_spec=self.env.action_spec,
            observation_spec=self.env.observation_spec,
        ...
        self.policy_white_1 = get_policy(
            name=cfg.algo.name,
            cfg=cfg.algo,
            action_spec=self.env.action_spec,
            observation_spec=self.env.observation_spec,
        ...
        if black_checkpoint1 := cfg.get("black_checkpoint1", None):
            self.policy_black_1.load_state_dict(
                torch.load(black_checkpoint1, map_location=self.cfg.device)
            )
            logging.info(f"black_checkpoint:{black_checkpoint}")
        if white_checkpoint1 := cfg.get("white_checkpoint1", None):
            self.policy_white_1.load_state_dict(
                torch.load(white_checkpoint1, map_location=self.cfg.device)
            )
            logging.info(f"white_checkpoint:{white_checkpoint}")
    ...
    def _get_baseline(self) -> _policy_t:
        # 1. ƯU TIÊN: Kiểm tra xem trong config có chỉ định file cụ thể không
        specific_ckpt = self.cfg.get("baseline_checkpoint", None)
        
        # Nếu có config và file đó tồn tại
        if specific_ckpt and os.path.isfile(specific_ckpt) and self.cfg.get("needbaseline", True):
            logging.info(f"Loading specific Baseline: {specific_ckpt}")
            baseline = get_policy(
            ...
            baseline.load_state_dict(torch.load(
                specific_ckpt, map_location=self.cfg.device))
            baseline.eval()
            return baseline

        # 2. DỰ PHÒNG: Nếu không chỉ định, dùng logic cũ (quét thư mục)
        ...
        if os.path.isdir(pretrained_dir) and (
        ...
        ) and self.cfg.get("needbaseline", True):
            baseline = get_policy(
                name=self.cfg.baseline.name,
                cfg=self.cfg.baseline,
                action_spec=self.env.action_spec,
            ...
            ckpts.sort()
            # Vẫn giữ logic cũ nếu không có config cụ thể
            logging.info(f"Baseline (Auto-selected): {ckpts[0]}") 
            baseline.load_state_dict(torch.load(
    ...
    def run(self, disable_tqdm: bool = False):
        ...
        self._post_run()
    def fake_run(self, disable_tqdm: bool = False):
        pbar = tqdm(range(self.epochs), disable=disable_tqdm)
        for i in pbar:
            info = {}
            info.update(self._epoch(epoch=i))
