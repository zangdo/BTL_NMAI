...
from gomoku_rl.collector import SelfPlayCollector, VersusPlayCollector, FakeCollector
import torch
...
class IndependentRLRunner_gay(Runner):
    def __init__(self, cfg: DictConfig) -> None:
        super().__init__(cfg)
        self._collector = FakeCollector(
            self.env, self.policy_black, self.policy_white,self.policy_white_1, self.policy_black_1, out_device=self.cfg.get("out_device", None), augment=self.cfg.get("augment", False),)

    def _epoch(self, epoch: int) -> dict[str, Any]:
        info = self._collector.rollout(self.steps)
        info = add_prefix(info, "versus_play/")
        info['fps'] = info['versus_play/fps']
        del info['versus_play/fps']
        if epoch % 50 == 0 and epoch != 0:
