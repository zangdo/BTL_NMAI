...
class GomokuBoard(QWidget):
    def __init__(
        ...
        self._env = GomokuEnv(num_envs=1, board_size=board_size, device="cpu")
    ...
    def current_player(self):
        turn = self._env.gomoku.turn.item()
        if turn == 0:
            return Piece.BLACK
        else:
            return Piece.WHITE
    ...
    @property
    def done(self):
        return self._env.gomoku.done.item()

    def _is_action_valid(self, action: int):
        return self._env.gomoku.is_valid(torch.tensor([action])).item()

    def _AI_step(self):
        if self.done:
            logging.warning(f"_AI_step:Game already done!!!")
            return        
        action: int = self.mcts.run(self._env)
        x = action // self.board_size
        y = action % self.board_size

        assert self._is_action_valid(action)

        logging.info(f"AI (MCTS): {self.current_player} ({x},{y})")
        self.step([x, y])
    ...
    def step(self, action: list[int]):
        ...
        self.tensordict.set("action", torch.tensor([x * self.board_size + y], device=self._env.device))
        self.tensordict = self._env.step(self.tensordict)
        self.mcts.advance_root(x * self.board_size + y)
...
@hydra.main(version_base=None, config_path=CONFIG_PATH, config_name="demo_MCTS")
def main(cfg: DictConfig):
    ...
    board = GomokuBoard(
        ...
        models=models,
        mcts=MCTS(
            policy_black=models[0] if len(models) > 0 else uniform_policy,
            policy_white=models[1] if len(models) > 1 else uniform_policy,
        )
    )
